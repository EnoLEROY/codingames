...
loop do
  ...
  message << "LINE #{closest_to_base.first[:id]} #{my_bases_cells.first[:id]} 1; MESSAGE ligne 275;"
  ...
  target_cristal_cells.each do |cell|
    ...
    message << "LINE #{cell[:id]} #{second_beacon} 1; MESSAGE ligne 283;"
    ...
    unless is_there_a_path_to_base?(cell, my_bases, cells, cells_with_my_ants)
      message_to_keep << "LINE #{cell[:id]} #{my_bases.first} 1; MESSAGE ligne 286;" unless ids_of_targeted_cells.include?(cell[:id])
