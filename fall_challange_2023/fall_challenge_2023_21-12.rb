...
loop do
  ...
  MY_DRONES.each do |drone_id, drone|
    ...
    if drone.creatures_scanned.length > 3 || possible_scan == 0 || (drone.direction == "SURFACE" && drone.creatures_scanned.length > 0) || (drone.y < 5000 && drone.creatures_scanned.length > 2 && turns > 10)
      move_x = drone.x
      move_y = 500
    ...
    if visible_monsters_nearby.length > 0 && drone.emergency == 0
      ...
      if closest_monsters.length == 1
        ...
        STDERR.puts "distance monster vecteur #{distance_monster_vecteur}"
        if angle_monster_target > 90 && closest_monster_distance > 1000
          move_x = target[:x]
          move_y = target[:y]
