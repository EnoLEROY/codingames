...
loop do
  ...
  CREATURES.each do |id, creature|
    creature.update_position(MY_DRONES)
  ...
  MY_DRONES.each do |drone_id, drone|
    ...
    CREATURES.each do |creature_id, creature|
      if creature.type == -1 && !creature.x.nil?
        ...
        if distance_monster_drone < 2000 && (drone.y < 8500 || drone.x > 1500 || drone.x < 8500)
          light = 0
        end
      end
    end
    ...
    if visible_monsters_nearby.length > 0
      ...
      if best_exit[:distance_to_target] == 10000 || too_close.length > 0 # pas de bonne exit trouvée
        ...
        STDERR.puts "closest monsters #{closest_monster_distance}"
        ...
        if closest_monsters.length == 1
          ...
          # distance entre vecteur point et le monstre
          distance_monster_vecteur = distance_point_vecteur({x: monster.x, y: monster.y}, {x: drone.x, y: drone.y}, {x: point[:x], y: point[:y]})
          if angle_monster_target > 120 && closest_monster_distance > 1000
            move_x = target[:x]
            move_y = target[:y]
          elsif angle_point_target < 90 && closest_monster_distance > 1000 && distance_monster_vecteur > 1000
            move_x = point[:x]
            move_y = point[:y]
          else
          ...
          # x = closest_monsters.first.x - drone.x
          # y = closest_monsters.first.y - drone.y

          # unit = Math.sqrt((drone.x - x)**2 + (drone.y - y)**2)
          # target_vector = vecteur_to_unitaire({x: target[:vector_x], y: target[:vector_y]}, unit)
          # vector_target_power = (vector_power / 90) * angle_entre_vecteurs({x: drone.x, y: drone.y}, {x: x, y: y}, {x: target[:x], y: target[:y]})
          # vector_target_power = 0 if (closest_monster_distance < 2100 && light == 1)
          # vector_target_power = 0 if (closest_monster_distance < 1500 && light == 0)
          # # vector_power fois depuis la projection a la droite monstre
          # move_x = (drone.x - closest_monsters.first.x)*vector_power + drone.x + target_vector[:x] * vector_target_power
          # move_y = (drone.y - closest_monsters.first.y)*vector_power + drone.y + target_vector[:y] * vector_target_power
