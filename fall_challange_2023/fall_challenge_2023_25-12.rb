...
loop do
  ...
  MY_DRONES.each do |drone_id, drone|
    ...
    # direction a l'aveugle ----------------------------------------------------
      ...
      # # prise en compte du type et de la couleur en fontion des poissons deja trouvés MARCHE PAS 
      # if found_creatures.length != 0
      #   type_color_found = {type: [], color: []}
      #   found_creatures.each do |creature_id|
      #     creature = CREATURES[creature_id]
      #     type_color_found[:type] << creature.type
      #     type_color_found[:color] << creature.color
      #   end
      #   type_priority =[type_color_found[:type].count(0), type_color_found[:type].count(1), type_color_found[:type].count(2)]
      #   STDERR.puts "type priority #{type_priority}"
      #   radar.each do |direction, creatures|
      #     creatures[0].map! do |fish_id|
      #       fish = CREATURES[fish_id]
      #       weight = type_priority[fish.type] + 1
      #       fish.type + 1 * weight # le type du poisson (plus haut type plus de points) * nombre de poissons trouvés si pas trouver * 1 sionon plus de poids
      #     end
      #   end
      # end
      # STDERR.puts "radar #{radar}"
