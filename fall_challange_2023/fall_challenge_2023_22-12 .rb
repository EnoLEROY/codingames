...
loop do
  ...
  MY_DRONES.each do |drone_id, drone|
    ...
    drone.battery < 15 ? light_frequance = 4 : light_frequance = 2
    ...
    if drone.creatures_scanned.length > 5 || possible_scan == 0 || (drone.direction == "SURFACE" && drone.creatures_scanned.length > 0) || (drone.y < 4000 && drone.creatures_scanned.length > 2 && turns > 10)
      move_x = drone.x
      move_y = 500
    ...
    if visible_monsters_nearby.length > 0 && drone.emergency == 0
      ...
      if closest_monsters.length == 3
        closest_monsters.each_with_index do |index, monster|
      ...
      if closest_monsters.length == 1 && closest_monsters.first.class == Creature
        ...
        angle_coefficient = angle_monster_target
        ...
        if angle_point_target < 90 && closest_monster_distance > 1000 && distance_monster_vecteur_point > 1000
          move_x = point[:x]
          move_y = point[:y]
        ...
        if closest_monsters.last.class == Creature && closest_monsters.first.class == Creature
          ...
          if projection_vector[:norme] < 200 && drone.direction != "SURFACE"
            if drone.y > 7500 && projection_vector[:y] > 0
              projection_vector[:y] = - projection_vector[:y]
            end
            if drone.x < 2500 && projection_vector[:x] < 0
          ...
          if projection_vector[:norme] < 200 && (drone.direction == "SURFACE" || drone.direction == "(0, 0)")
